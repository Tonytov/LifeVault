...
/**
 * Тест ограничения длины поля роста (максимум 3 цифры)
 */
class OnboardingHeightFieldLimitTest : TestCase(
    ...
    @Test
    fun testHeightFieldLimitedTo3Digits() = run {
        step("Вводим 3 цифры") {
            onComposeScreen<OnboardingScreen>(composeTestRule) {
                heightField.performTextInput("123")
            }
        }

        step("Пытаемся ввести еще одну цифру") {
            onComposeScreen<OnboardingScreen>(composeTestRule) {
                heightField.performTextInput("4")
            }
        }
